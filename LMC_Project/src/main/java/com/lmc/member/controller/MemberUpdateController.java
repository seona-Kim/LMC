	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			if(multiRequest.getOriginalFileName("updateImg") != null) {
				memberImg = savePath + memberNo + "." + changeImg.substring(changeImg.lastIndexOf(".") + 1);
				new File(memberImg).delete();
				new File(savePath + changeImg).renameTo(new File(memberImg));
