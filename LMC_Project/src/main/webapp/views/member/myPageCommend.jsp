	...
	<div class="wrapBody">
          <div id="body">
              ...
              <div id="body_2">
                   <div class="outer">
			        <br>
			        <h2 align="center">내가 추천한 게시글</h2>
			        <br>
			        <form action="<%= contextPath %>/deleteMyCommend.bo" method="post">
			        <table class="list-area" align="center" border="1">
			            ...
			            <tbody>
		                	...
		                	<% } else { %>
			            		<% for(Board b : list) { %>
		                			<tr>
		                				<td class="notSelect"><input type="checkbox" name="deleteCNo" value="<%= b.getCommendNo() %>"></td>
		                				<td><%= b.getBoardNo() %></td>
		                				<td><%= b.getCategory() %></td>
			        ...
			        <div style="width:770px;" align="right">
						<button type="submit" class="btn btn-warning btn-sm" name="deleteMyCNo">삭제하기</button>
						<br><br>
					</div>
			        <br>
			        </form>
			        ...
			        <script type="text/javascript">
			        	$(function(){
			        		$("input[name='deleteAll']").change(function(){
			        			$("input[name='deleteCNo']").prop('checked', $("input[name='deleteAll']").is(":checked"));
			        		});
			        		
			        		$("input[name='deleteCNo']").click(function() {
			        			if($("input[name='deleteCNo']:checked").length != $("input[name='deleteCNo']").length){
			        		...
			        		$(".list-area>tbody>tr").children().not(".notSelect").click(function() {
								let cno = $(this).parent().children().eq(1).text();
								location.href = "<%= contextPath %>/detail.bo?bno=" + cno;
							});
			        		
			        		$("button[name='deleteMyCNo']").click(function() {
			        			var result = confirm($(".list-area>tbody input[name='deleteCNo']:checked").length + "개의 추천을 삭제하시겠습니까?");
								return result;
			        		});
			        	});
			        </script>
